...
//            total + meal.dishServings.reduce(0) { $0 + ($1.protein ?? 0) }
//        }
//    }
//
//    var totalFat: Double {
//        meals.reduce(0) { total, meal in
//            total + meal.dishServings.reduce(0) { $0 + ($1.fat ?? 0) }
//        }
//    }
//
//    var totalCarbs: Double {
//        meals.reduce(0) { total, meal in
//            total + meal.dishServings.reduce(0) { $0 + ($1.carbs ?? 0) }
//        }
//    }
//
//    init(date: Date) {
    ...
    var body: some View {
        VStack(spacing: 15) {
            ...
            Divider()
                ...
                .frame(height: 1)
            HStack(spacing: 0) {
                ComponentView(title: "Protein", current: Int(totalProtein), total: 10, color: Color("protein"))
                    .frame(minWidth: 50, maxWidth: .infinity)
                ComponentView(title: "Carbs", current: Int(totalCarbs), total: 10, color: Color("carbs"))
                    .frame(minWidth: 50, maxWidth: .infinity)
                ComponentView(title: "Fat", current: Int(totalFat), total: 10, color: Color("fat"))
                    .frame(minWidth: 50, maxWidth: .infinity)
            }
        }
    }
